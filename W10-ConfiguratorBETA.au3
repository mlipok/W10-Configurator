...
=======
	$RZColCount = UBound($sRZCatalog,1)-1
	_ArrayDelete($sRZCatalog, $RZColCount) ;Removes the blank line at the end
	If ISArray($sRZCatalog) = True Then 
		_ArraySort($sRZCatalog)
>>>>>>> 4e146229f0c468857437200c204e23c32d77e42c:W10-ConfiguratorBETA.au3
		c("Catalog updated.")
	Else
...
Func GUI()
	...
	GUICtrlCreateTabItem("RZGet Programs")
...
$task[1] =  GuiCtrlCreateCheckbox("Windows Updater", 710, 48, 260, 20, BitOR($TVS_DISABLEDRAGDROP,$TVS_CHECKBOXES))
	GUICtrlSetFont(-1,12,400,0,"Lucida Bright", 5)
...
GUICtrlSetColor(-1, 0x75EE3B)
 ...
 ;Tab for selecting programs installed by RuckZuck tool
 _RZCatalog()
 GUISwitch($GUI,_GUICtrlTab_SetCurFocus($tab,3)&GUICtrlRead ($tab, 1))
	GUICtrlCreateLabel("Select the applicaitons below that you would like installed via the RuckZuck tool.", 16, 30,600,40)
		GUICtrlSetFont(-1,11,400,0,"Lucida Bright", 5)
		GUICtrlSetResizing(-1,$GUI_DOCKAUTO)
	GUICtrlCreateLabel('To update the below list select the ' & '"' & 'Update Catalog' & '"' & ' button to the right of the box.', 16, 50,580,40)
		GUICtrlSetFont(-1,11,400,0,"Lucida Bright", 5)
		GUICtrlSetResizing(-1,$GUI_DOCKAUTO)
	$RZCatUpd = GUICtrlCreateButton($sRZCatUpd,530,80,150,30,-1,-1)
		GUICtrlSetFont(-1,11,400,0,"Lucida Bright", 5)
		GUICtrlSetResizing(-1,$GUI_DOCKAUTO)
	Global $idListview = GUICtrlCreateListView(" | Application Name", 16, 75, 500, 450, -1, BitOR($LVS_EX_CHECKBOXES,$WS_EX_CLIENTEDGE))
		;GUICtrlCreateListViewItem(" |All", $idListview) ;Disabled until checking of what is selected is completed
		Global $Test = 0
		ConsoleWrite("Test is: " & $Test & @LF)		
		Global $RZCheck
		For $i = 0 To $RZColCount - 2 Step 1
			ConsoleWrite("Test before is: " & $Test & @LF)
			;$Test = ($Test-46) + 1
			$Test = GUICtrlCreateListViewItem(" |"& $sRZCatalog[$i], $idListview)
		Next
...
;~ 			c("Configuration started!")
		...
		Case $RZCatUpd
			_RZCatalog()
			_ArrayDisplay($idListview)
			GUICTrlSetData($RZCatUpd, "Update Catalog")
	EndSwitch
Wend
